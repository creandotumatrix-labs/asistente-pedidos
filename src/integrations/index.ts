...
import { upsertContact, hubspotConfigured } from "./hubspot.ts";
...
export function wireIntegrations(): void {
  ...
  bus.on("feed", (e: FeedEvent) => {
    ...
    switch (e.type) {
      ...
      case "reservation": {
        // Restaurant table booking → calendar event.
        void createCalendarEvent({
          summary: `Reservación — ${s("name") ?? ""} (${p.size ?? "?"}p)`,
          description: `${biz} · Tel: ${s("tel") ?? ""}`,
          when: s("datetime"),
        });
        void upsertContact({ firstname: s("name"), phone: s("tel"), note: `Reservación mesa (${p.size ?? "?"}p)` });
        break;
      }
      case "lead": {
